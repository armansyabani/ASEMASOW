...
async function checkVersionUpdate() {
  try {
    ...
    const url = 'https://raw.githubusercontent.com/AhmadAkbarID/hydromd/refs/heads/master/package.json';
    const response = await axios.get(url);
    const remotePkg = response.data;
    const remoteVersion = remotePkg.version;

    if (remoteVersion !== localVersion) {
      console.log(color(`[INFO] Terdeteksi adanya update script: v${remoteVersion}.\nSilakan update di https://github.com/AhmadAkbarID/hydromd`, 'yellow'));
    } else {
